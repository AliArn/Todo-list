  ...
  return (
    ...
    <div className="mx-3 md:container md:mx-auto my-5 rounded-xl p-5 bg-violet-100 min-h-[80vh] md:w-[35%]">
      ...
      <div className="addTodo my-5 flex flex-col gap-5">
        ...
        <button onClick={handleAdd} disabled={todo.length<=3} className='bg-violet-800 mx-2 rounded-full hover:bg-violet-950 disabled:bg-violet-700 p-3 py-1 text-sm text-white'>Save</button>
        </div>
        
      </div>
         <input className='my-4' id='show' onChange={toggleFinished} type="checkbox" checked={showFinished} /> 
         <label className='mx-2' htmlFor="show">Show Finished</label>
         <div className='h-[1px] bg-black opacity-100 w-[90%] mx-auto my-2'></div>
         ...
         return (showFinished || !item.isComleted) && <div key={item.id} className="todo flex my-3 justify-between">
          ...
          <div className={item.isComleted?"line-through":""}>{item.todo}</div>
            <div className="buttons flex h-full">
              <button onClick={(e)=>handleEdit(e, item.id)} className='bg-violet-800 hover:bg-violet-950 p-2 py-1 text-sm text-white rounded-md mx-1'><FaEdit /></button>
